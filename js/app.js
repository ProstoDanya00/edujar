    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".partners__slider")) new core(".partners__slider", {
                ...
                speed: 800,
                navigation: {
                    prevEl: ".arrows-partners__button_prev",
                    nextEl: ".arrows-partners__button_next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            if (document.querySelector(".testimonials__slider")) new core(".testimonials__slider", {
                ...
                spaceBetween: 55,
                navigation: {
                    prevEl: ".arrows-testimonials__arrow_prev",
                    nextEl: ".arrows-testimonials__arrow_next"
                },
                on: {}
            });
        }
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.classList.contains("tab-navigation-courses__button") && !targetElement.classList.contains("_active")) {
                const activeButton = document.querySelector(".tab-navigation-courses__button._active");
                const filterElements = document.querySelectorAll(".courses__item");
                ...
                if (filterElements.length) {
                    const tabHeader = document.querySelector(".courses__header");
                    filterElements.forEach((filterElement => {
                        if (filterElement.dataset.filterType === filterType) {
                            filterElement.classList.add("_filter-active");
                            filterElement.style.display = "flex";
                            initCloseButton(tabHeader, targetElement);
                        } else filterElement.style.display = "none";
                    }));
                }
            }
        }
        function initCloseButton(tabHeader, targetElement) {
            const tabNavigation = tabHeader.querySelector(".courses__tab-navigation");
            const tabCloseButton = tabHeader.querySelector(".courses__close");
            if (targetElement.classList.contains("_active")) {
                tabNavigation.classList.add("filter-active");
                tabCloseButton.classList.add("_close");
                tabCloseButton.addEventListener("click", closeFilter);
            }
        }
        function closeFilter(e) {
            const targetElement = e.target;
            const tabHeader = targetElement.closest(".courses__header");
            const tabNavigation = tabHeader.querySelector(".courses__tab-navigation");
            const tabActiveButton = tabHeader.querySelector(".tab-navigation-courses__button._active");
            const closeButton = tabHeader.querySelector(".courses__close");
            if (tabActiveButton.classList.contains("_active") && tabNavigation.classList.contains("filter-active")) {
                tabNavigation.classList.remove("filter-active");
                tabActiveButton.classList.remove("_active");
                closeButton.classList.remove("_close");
                closeBody(tabHeader);
            }
        }
        function closeBody(tabHeader) {
            const tabContainer = tabHeader.closest(".courses__container");
            const tabItems = tabContainer.querySelectorAll(".courses__item");
            if (tabItems.length) tabItems.forEach((tabItem => {
                tabItem.classList.remove("_filter-active");
                tabItem.style.cssText = "flex";
            }));
        }
        function svgAnim() {
            const path = document.querySelectorAll("[data-svg-anim] path");
            if (path.length) path.forEach((el => {
                const pathLength = getTotalLength(el);
                el.style.cssText = `\n                stroke-dasharray: ${pathLength};\n                stroke-dashoffset: ${pathLength};\n            `;
            }));
            function getTotalLength(svg) {
                return Math.round(svg.getTotalLength());
            }
        }
        window["FLS"] = true;
        isWebp();
